const instruct = (n = 3, us = 1, them = 1, advUs = 0, advThem = 0) => {
  const nData = orunderDataStandAlone.filter((x) => {
    return x.n === n;
  });
  const actualFR = parseFloat((us / them).toPrecision(4));

  //FILTER HERE for d100
  const diceData = nData;
  ...
  return `${n}d${encounter[0].d}<small> =orUnder </small>${encounter[0].t}`;
};
...
const refresh = ({ n, us, them, advUs, advThem }: State) => {
  const ouput = document.querySelector("output") as HTMLElement;
  ouput.innerHTML = instruct(n, us, them, advUs, advThem);
